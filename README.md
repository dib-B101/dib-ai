...
| 이상거래 탐지 — ML 트랙 | 섀도 모드 가동 (`w_ml=0`, 점수만 기록) | `src/fraud_ml/`, `src/fraud/ml_features.py` |
| 상품 검수 — 1차 규칙 필터 | 구현 완료 | `src/moderation/` |
| 상품 검수 — 2차 AI 검수 | 실호출 검증 완료 (7/7) | `src/moderation/llm.py` |
...
```bash
FRAUD_W_RULE=0.7
FRAUD_W_ML=0.3      # 기본값은 0. 켜야 모델이 판정에 반영된다
```
...
`w_ml=0` 인 채로 모델을 아예 안 부르면 **나중에 켤 근거가 영영 안 생긴다.** "검증되면
켠다" 고만 두면 검증할 방법이 없다. 그래서 기본값으로 **점수는 계산하되 판정에는
섞지 않는다.**

```bash
FRAUD_ML_SHADOW=true   # 기본값. 끄려면 false
```

```
 member     rule       ml     risk  band
     21   0.5353   0.5385   0.5353  medium   ← 두 트랙이 같은 결론
     22   0.3936   0.4571   0.3936  medium
     23   0.0333   0.0059   0.0333  low      ← 우연히 낀 일반 입찰자
```

`risk_score` 가 `rule_score` 와 **정확히 같다.** `combine()` 이 `w_ml<=0` 이면 규칙
점수를 그대로 돌려주기 때문이다 — 식으로 계산해도 값은 같지만 나눗셈에서 끝자리가
흔들리면 "가중치를 0 으로 뒀는데 점수가 왜 달라졌지" 를 쫓게 된다.

응답에서 구분하는 법.

| 필드 | 뜻 |
| --- | --- |
| `ml_score` | 모델이 낸 점수. 섀도여도 채워진다 |
| `model_version` | **그 점수를 만든 모델.** `ml_score` 와 같이 있거나 같이 없다 |
| `ml_shadow` | true 면 계산만 했고 판정에는 안 썼다 |
| `weights.w_ml` | 판정에 실제로 들어간 비율 |

명세 93 콜백에는 `weights` 자리가 없지만 `ruleScore` · `mlScore` · `riskScore` 가 따로
담기므로, **`riskScore == ruleScore` 면 모델이 판정에 안 쓰인 것**으로 읽으면 된다.

### 켜는 순서

숫자를 한 번에 올리지 않는다. `w_ml` 이 올라가면 **HIGH 밴드 경계를 넘나드는 사람이
생기고, 그때부터는 실제 제재 대상이 바뀐다.**

| 단계 | 조건 | 값 |
| --- | --- | --- |
| 1 | 지금 — 섀도로 불일치 사례를 모은다 | `0.0` |
| 2 | 운영자 판정 30~50건 | `0.0` |
| 3 | 모델이 룰보다 나쁘지 않음을 확인 | `0.1` |
| 4 | 밴드가 뒤집힌 케이스를 검토해 모델 쪽이 맞았을 때 | `0.3` |

3단계를 `0.1` 로 시작하는 이유는 점수가 움직이는 것은 보면서도 밴드는 거의 안
뒤집히기 때문이다. 변화를 관찰할 수 있으면서 피해는 없는 구간이다.
...
- **DIB 라벨 데이터가 없어 Precision/Recall 은 재지 못했다.** 아래 실호출은 기능이
  도는지를 본 것이지 정확도를 잰 것이 아니다.

## 실호출 검증

`gemini-3.5-flash` 로 실제 호출해 확인했다 (`scripts/check_moderation_llm.py`).

```
7/7 일치 · 총 19.8초 (건당 평균 2.8초)
```

| 상품명 | 무엇을 보는가 | 단계 | 걸린 시간 |
| --- | --- | --- | --- |
| 아이폰 15 프로 256GB | 평범한 상품을 막지 않는가 | ai | 3.3초 |
| 전자담배 액상 팝니다 | 1차 규칙이 AI 없이 잡는가 | **rule** | **0.0초** |
| 행복한 연기 팝니다 | 금칙어 없는 우회 표현을 문맥으로 읽는가 | ai | 3.1초 |
| 어른들의 음료 한 박스 | 우회 표현 2 — 주류 | ai | 3.0초 |
| 명품 가방 판매 | 부정문("레플리카 아닙니다")에 속지 않는가 | ai | 4.1초 |
| 캠핑용 나이프 | 부정문 2 | ai | 3.1초 |
| 빈티지 재떨이 | 연상시킬 뿐인 물건을 막지 않는가 | ai | 3.1초 |

**규칙이 잡은 건만 0.0초다.** 명백한 위반에 LLM 비용을 쓰지 않는다는 설계가 그대로
나타난다. 나머지는 3초대이므로, 사전을 넓히는 일은 정확도뿐 아니라 비용·지연 문제이기도 하다.

### 이미지가 실제로 모델에 닿는가

응답이 오는 것만으로는 이미지가 쓰였다고 말할 수 없다. 그래서 **상품명을 일부러
중립으로 두고** 텍스트만 / 텍스트+이미지를 각각 호출해 비교했다
(`scripts/check_moderation_image.py`).

```
[텍스트만]   검토 필요 (conf 0.95)
             "상품명과 설명에 구체적인 상품 정보가 기재되어 있지 않아 판정할 수 없습니다"

사진 A       검토 필요 → 정상       의류/패션잡화 · conf 0.95
사진 B       검토 필요 → 정상       의류/패션잡화 · conf 0.90
사진 C       검토 필요 → 검토 필요   conf 0.85
```

**3건 중 2건이 판정을 바꿨다.** 나머지 1건은 원단을 극단적으로 확대한 클로즈업이라
사람이 봐도 무엇을 파는지 알 수 없는 사진이었는데, 모델의 사유가 이렇게 바뀌었다.

```
이미지 없음   "상품명과 설명에 구체적인 상품 정보가 기재되어 있지 않아"
이미지 있음   "등록된 사진과 설명만으로는 구체적인 상품 종류를 식별하기 어려워"
```

**사진을 보고 나서 모르겠다고 한 것이다.** 확신도도 0.95 에서 0.85 로 내려갔다.
이미지가 전달된다는 것과, **판단이 서지 않을 때 억지로 결론을 내지 않는다는 것**을
함께 확인한 셈이다.
...
| 라벨 데이터 확보 후 | Precision/Recall 측정. 지금은 기능 동작만 확인했다 |
...
```bash
DIB_DATABASE_URL=... DIB_DB_TIMEZONE=UTC python scripts/check_db.py
```

가짜 커넥션 테스트로는 **못 잡는 것**만 본다 — SQL 문법, 컬럼명 오타, psycopg 배열
파라미터, `::vector` 캐스팅, 드라이버가 벡터를 리스트로 주는지 문자열로 주는지.

실제로 이걸로 버그를 하나 잡았다. 추천 엔드포인트가 `datetime.now(timezone.utc)` 로
비교하는데 DB 의 `TIMESTAMP` 는 시간대가 없어, 붙이는 순간 500 이 났다. 행 변환만
검증하고 랭킹까지 이어 보지 않아서 놓쳤던 것이다.

### 시간대를 반드시 맞춰야 한다

`DIB_DB_TIMEZONE` 이 **DB 의 시간대 없는 값이 어느 지역 시각인지**를 정한다.
안 맞추면 진행 중인 경매가 전부 "종료됨" 으로 걸러져 **추천이 빈 목록으로 나간다.**
예외도 로그도 없다.

로컬 `docker-compose` 의 Postgres 는 `Etc/UTC` 로 뜬다. 운영 DB 가 어느 시간대로
저장하는지는 백엔드에 확인해야 한다.
...
```
GET /internal/reco/home?member_id=1234
```

`member_id` 를 주면 **행동 로그로 관심 프로필을 만들어** 순서를 바꾼다. 로그가 없으면
자동으로 인기순으로 떨어지므로 신규 사용자에게도 안전하다. 실제로 개인화가 적용됐는지는
응답의 `strategy` 로 확인한다 (`personalized` / `popularity`).

## 찜은 `member_event` 가 아니라 `bookmark` 에 있다

백엔드가 `member_event` 에 기록하는 이벤트는 현재 **`BID` 하나뿐**이다. 그런데 찜은
별도의 `bookmark` 테이블에 **이미 쌓이고 있고**, 프로필을 만드는 데 필요한 것
(`member_id` · `product_id` · `created_at`)이 전부 있다.

그래서 **백엔드 작업을 기다리지 않고 그 테이블을 직접 읽는다.** 입찰까지 간 상품은
사람당 몇 개 안 되므로, 찜(가중치 4.0)이 붙어야 프로필이 의미를 갖는다.

두 가지를 조심한다.

**`bookmark` 는 상품 단위인데 프로필은 경매 단위다.** 유찰 후 재등록되면 한 상품에
경매가 여러 개 붙으므로 **상품마다 가장 최근 경매 하나**만 쓴다. 벡터는 어느 경매를
골라도 같고(상품이 같으니), 지금 보고 있을 경매는 최신 쪽이다.

**같은 찜을 두 번 세지 않는다.** 백엔드가 나중에 `member_event` 에도 `WATCH` 를
넣으면 같은 찜이 양쪽에서 들어온다. 그러면 가중치가 4.0 이 아니라 8.0 이 되어
**찜 하나가 입찰보다 강해진다.** 이미 로그에 있는 경매는 `bookmark` 쪽을 버린다.

`bookmark` 는 **현재 찜 상태이지 이력이 아니다.** 찜을 해제하면 행이 사라져
`UNWATCH` 는 잡히지 않는다. 다만 해제된 건 애초에 안 잡히므로 결과는 같다.

## 프로필 만들기

```
member_event (BID) + bookmark (찜) 조회 (최근 90일, 최대 500건)
    ↓ 행동 가중치     입찰 6 · 찜 4 · 검색·클릭·공유 2 · 조회 1
    ↓ 시간 감쇠       반감기 14일. 0.5 ^ (경과일 / 14)
경매별 최종 가중치
    ↓ 해당 상품 벡터를 가중 평균 → L2 정규화
관심 프로필 (텍스트 · 이미지)
    ↓ 후보와 유사도 (유사 상품과 같은 결합 방식)
rank(similarity=..., strategy="personalized")
```

**반감기를 쓰는 이유**가 있다. 중고 거래는 필요가 생겼다 사라지는 주기가 짧다. 한 달
전에 냉장고를 찾아본 사람에게 지금도 냉장고를 보여주면 이미 산 물건을 계속 권하는
셈이 된다. 선형 감쇠 대신 지수를 쓴 것은 오래된 행동이 **어느 순간 0 이 되어 사라지는**
것을 막기 위해서다 — 로그가 적은 초기에는 옛 행동도 있어야 한다.

같은 상품에 여러 행동이 있으면 **더한다.** 보고(VIEW) 찜하고(WATCH) 입찰한(BID)
상품은 셋 중 하나만 한 상품보다 관심이 크다.

## 이미 관심을 보인 경매는 뺀다

입찰하거나 찜한 상품은 그 사람이 이미 아는 상품이고 마이페이지에서 따로 본다.
**프로필이 그 상품들의 평균이라 유사도가 당연히 최상위로 나오므로, 안 빼면 추천
목록이 이미 본 상품으로 채워진다.** 제외한 건수는 `excluded.already_seen` 에 담긴다.

실DB 로 확인한 결과다.

```
비로그인    15 쿠쿠밥솥 · 6 맥북 · 7 에어팟 · 13 다이슨 · 11 애플워치
회원 1      6 맥북 · 5 아이패드 · 8 갤럭시탭 · 7 에어팟 · 13 다이슨
            (아이폰 13 프로에 입찰·찜·조회)   excluded.already_seen = 1
```

취향 점수가 아이패드 1.000 · 갤럭시탭 0.923 · 맥북 0.846 · 에어팟 0.769 · 다이슨 0.538
로 나왔다. 생활가전이 상위에서 밀려났다.
...
| --- | --- |
| 이벤트가 없다 | 신규 사용자 |
| 행동한 상품에 임베딩이 없다 | 배치가 아직 안 돔 |
| 양의 가중치가 남지 않았다 | 음의 신호만 있다 |

셋 다 **0 벡터를 만들지 않는다.** 0 벡터는 모든 상품과 유사도 0 이 되어 "아무것도 안
좋아함" 으로 읽히는데, 그건 **"모름" 과 다르다.** `None` 을 돌려주면 인기순으로 떨어진다.

행동 로그 **조회 실패도 예외로 올리지 않는다.** 개인화는 있으면 좋은 것이지 없으면
추천이 안 나가는 것이 아니다. 추천 전체가 503 이 되는 편이 훨씬 나쁘다.

## 음의 신호를 조심해서 쓴다

찜 해제(`UNWATCH`)는 이전 찜을 되돌리는 명시적 행동이라 같은 크기로 뺀다. 반면 스크롤로
지나친 것(`SCROLL_PASS`)은 관심이 없어서일 수도, **이미 아는 상품이어서일 수도** 있다.
한 번 지나쳤다고 싫어한다고 보면 오탐이 쌓이므로 −0.5 만 준다.

## 개인화 비중이 유사 상품보다 낮은 이유

```
유사 상품   0.5    "지금 이걸 보고 있다" — 확실한 신호
개인화      0.4    과거 행동에서 추정한 값 — 덜 확실하다
```

## 알려진 제약

- `member_event` 의 시각 컬럼은 **`occured_at`** 이다. `occurred_at` 이 아니다 —
  백엔드 스키마의 오타이고 엔티티에도 그대로 매핑돼 있어 맞춤법을 고치면 조회가 깨진다
- 백엔드가 `member_event` 에 기록하는 이벤트는 **`BID` 하나**다. 찜은 `bookmark`
  테이블에서 직접 읽는다. `VIEW`(조회)·`SEARCH` 는 **아무 데도 쌓이지 않아** 백엔드가
  만들어야 한다
- `dwell_ms` 는 ERD 에만 있고 마이그레이션에는 없다. 라이브 체류시간 기반 관심도는
  그 컬럼이 생겨야 만들 수 있다
- 가중치·반감기는 근거 없는 초기값이다. 클릭 로그가 쌓이면 조정한다
...
`embedded_at` 컬럼이 없어도 알 수 있다. 근거는 백엔드 구현에 있다.

```java
// ProductCommandServiceImpl.java:129 — 상품 수정 시
if (updated) {
    product.setStatus(ProductStatus.PENDING);
}
```

**상품을 수정하면 반드시 `PENDING` 으로 돌아간다.** 재검수를 거쳐야 하기 때문이다.
그래서 `PENDING` 이면 곧 "내용이 바뀌었다" 는 뜻이다.

```
수정 → PENDING → 배치가 벡터를 비움 → 재검수 통과 → 배치가 새로 계산
```

실행할 때마다 먼저 **`PENDING` · `REJECTED` · 삭제된 상품의 벡터를 비운다.**
`NULL` 이 곧 "다시 계산해야 함" 이므로 따로 기록할 것이 없다. 거부·삭제된 상품의
벡터를 남겨 두지 않는 효과도 같이 얻는다 — 남아 있으면 노출 정책이 바뀔 때 조용히
추천에 섞여 들어간다.

비운 상품을 같은 실행에서 다시 계산하지는 않는다. `PENDING` 은 계산 대상이 아니다.
(`--no-reclaim` 으로 이 단계를 끌 수 있다.)

## 남은 한계 — 모델을 바꿨을 때

어느 행이 어떤 모델로 계산됐는지는 DB 에 없다. 모델이나 텍스트 구성을 바꾸면
`--all` 로 전부 다시 돌리는 수밖에 없다. **드물고 의도적인 작업**이라 지금은 이걸로
...
**`memberId` 를 주면 개인화된다.** 동기 엔드포인트(`/internal/reco/home`)와 **같은
함수**를 쓰므로 두 경로의 결과가 갈라지지 않는다 — 갈라지면 같은 회원이 문에 따라
다른 추천을 받고, 백엔드가 한쪽만 붙였을 때 "왜 개인화가 안 되지" 를 디버깅하게 된다.

**`behaviorWindow` 는 쓰지 않는다.** 조회 구간은 `config/reco.yaml` 의
`lookback_days` 가 정한다 — 요청마다 구간이 달라지면 같은 회원의 추천이 호출자에
따라 바뀐다. 거절하면 연동이 깨지므로 받아만 두고 무시한다.
...
**이 단계에는 개인화가 없다.** 마감 임박도·인기도·경쟁도만으로 순서를 만든다.
개인화는 STEP 4·5 에서 같은 엔드포인트에 얹었다 — 위의 `추천 (STEP 4·5 — 개인화)` 를 보라.
...
개인화가 붙은 지금도 **엔드포인트 계약은 바뀌지 않았다.** 응답의 `strategy` 값만 달라진다.
`member_id` 를 처음부터 받아 둔 덕분에 백엔드 연동을 두 번 하지 않았다.

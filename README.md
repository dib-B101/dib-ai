...
### 백분위 정규화를 빠뜨리면 안 되는 이유

두 유사도의 **분포가 다르다.** 실측에서 텍스트 유사도는 0.36~0.70 에 몰려, 무관한 상품
끼리도 0.4 대가 나왔다. 원값을 그대로 가중합하면 차이가 작은 쪽의 **가중치가 이름뿐인
숫자**가 된다.

대신 돌려주는 유사도는 **후보 집합 안에서의 상대 순위이지 "이만큼 닮았다" 는 절대값이

...
def test_blend_uses_rank_not_raw_value():
    """원값이 아니라 백분위를 섞는다.

    텍스트 유사도가 0.68/0.66/0.64 처럼 좁은 구간에 몰려도 순위가 벌어져야 한다.
    원값을 그대로 가중합하면 차이가 0.02 라 다른 항에 묻힌다.
    """
...
def test_missing_image_falls_back_to_text_only():
    ...
    assert out[2] == pytest.approx(1.0), "텍스트 백분위 그대로여야 한다"
